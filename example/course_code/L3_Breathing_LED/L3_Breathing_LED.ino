...
// 定义 ADC 连接的引脚编号为 28，后续可通过该宏来引用此引脚
#define ADC_pin 28
// 定义红色 LED 连接的引脚编号为 18，后续可通过该宏来引用此引脚
#define Red_LED 18
// 定义一个整型变量，用于存储从 ADC 引脚读取到的模拟值
int adcValue;
// 定义一个整型变量，用于存储将 ADC 模拟值映射后的结果
int mappedValue;

// 此函数用于进行 L3 相关功能的初始化操作，仅在开始时执行一次
void L3_init() {
    // 将 ADC 引脚设置为输入模式，以便能够读取该引脚上的模拟信号
    pinMode(ADC_pin, INPUT);
    // 将红色 LED 引脚设置为输出模式，以便能够控制该引脚输出不同的电平来点亮或熄灭 LED
    pinMode(Red_LED, OUTPUT);
    // 将引脚 14 设置为输入模式，可能用于接收某个控制信号
    pinMode(14, INPUT);
}

// 此函数用于执行 L3 课程相关的具体逻辑，会在满足条件时循环执行
void L3_Curriculum() {
    // 调用 L3_init 函数进行初始化操作
    L3_init();

    // 进入一个无限循环，持续执行课程逻辑
    while (1) {
        // 从 ADC_pin 引脚读取模拟值，并将其存储到 adcValue 变量中
        adcValue = analogRead(ADC_pin);

        // 判断读取到的 ADC 值是否小于 51（假设 ADC 满量程值为 1023，51 约为 1023 的 5%）
        if (adcValue < 51) {
            // 如果小于 51，则将红色 LED 引脚的输出电平设置为 0，即熄灭 LED
            analogWrite(Red_LED, 0);
        } else {
            // 如果大于等于 51，则将读取到的 ADC 值从 0 - 1023 的范围映射到 0 - 255 的范围
            // 这是因为在使用 analogWrite 控制 LED 亮度时，输入值范围为 0 - 255
            mappedValue = map(adcValue, 0, 1023, 0, 255);
            // 根据映射后的结果，通过 PWM 方式控制红色 LED 的亮度
            analogWrite(Red_LED, mappedValue);
        }

        // 延时 10 毫秒，避免过于频繁地读取 ADC 值和更新 LED 亮度
        delay(10);

        // 读取引脚 14 的电平状态，如果为高电平
        if (digitalRead(14)) {
...
// setup 函数在程序启动时仅执行一次，用于完成各种初始化操作
void setup(void) {
    // 初始化串口通信，设置波特率为 115200，用于与外部设备进行数据传输和调试
    Serial.begin(115200);

    // 将 LED_EN 引脚设置为输出模式，以便后续控制与之相连的 LED 灯的亮灭
    pinMode(LED_EN, OUTPUT);

    // 初始化 NeoPixel 灯带对象，这是使用 NeoPixel 灯带所必需的步骤
    strip.begin();
    ...
    // 设置 I2C 通信的 SDA 引脚为 4，用于数据传输
    Wire.setSDA(4);
    // 设置 I2C 通信的 SCL 引脚为 5，用于时钟信号传输
    Wire.setSCL(5);
    // 初始化 I2C 通信，使其可以正常工作
    Wire.begin();
    ...
    gfx.init();
    // 初始化 LittlevGL 图形库，这是一个用于创建图形用户界面（GUI）的库
    lv_init();
    ...
    // 调用 L3_init 函数，进行与 L3 相关的初始化操作
    L3_init();
}

// loop 函数会在 setup 函数执行完成后不断循环执行，用于处理各种周期性的任务
void loop() {
    // 调用 LittlevGL 的定时器处理函数，处理界面的定时更新和动画等操作
    lv_timer_handler();
    ...
    // 判断课程模式标志位 Curriculum_flag 是否为 1，如果为 1 则进入课程模式
    if (Curriculum_flag == 1) {
        // 将引脚 0 置为低电平，关闭背光
        digitalWrite(0, LOW);
        // 调用 L3_Curriculum 函数，执行课程模式的具体逻辑
        L3_Curriculum();
        // 将引脚 0 置为高电平，开启背光
        digitalWrite(0, HIGH);
    } else {
        // 如果课程模式标志位为 0，将红色 LED 引脚置为低电平，关闭红色 LED
